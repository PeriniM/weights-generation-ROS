...
# x(t) = a + bt + ct^2 + dt^3 -> funzione del terzo ordine da interpolare 

def compute_3rd_order_poly_traj(x0, x1, T, dt):
    dim = x0.size #define the dimention of the input 
    ...
    A = np.array([[1, 0, 0, 0],
                ...
                [0, 1, 2*T, 3*T**2]])
    for s in range(dim):
        # b -> [P0, P1, P0_zerp_vel, P1_zero_vel]
        b = np.array([x0[s], x1[s], 0, 0]).transpose()
        # compute poly_coeff = A^-1*b
